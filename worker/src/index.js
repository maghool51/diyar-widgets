...
/**
 * Returns ONLY the current online-visitor count, with no other data and no
 * authentication — this is intentionally the one piece of `/aggregate`'s
 * output that is safe to expose publicly (no visitor_hash, IP, timestamp,
 * or any other row-level D1 data ever leaves this function). Read-only:
 * issues a single SELECT and nothing else.
 *
 * Reuses the exact same online-window computation `handleAggregate` already
 * uses (same `ONLINE_WINDOW_MS`, same SQL) rather than reimplementing it.
 *
 * @param {Request} request
 * @param {{DIYAR_DB: import('@cloudflare/workers-types').D1Database}} env
 * @returns {Promise<Response>}
...
async function handleOnline(request, env) {
  ...
  let online = 0;
  try {
    const onlineRow = await env.DIYAR_DB
      .prepare('SELECT COUNT(*) AS n FROM sessions WHERE last_seen > ?')
      .bind(onlineThreshold)
      .first();
    online = onlineRow?.n ?? 0;
  } catch (error) {
    // Never leak D1/Worker internals to a public, unauthenticated caller —
    // log server-side only (visible in `wrangler tail`) and still respond
    // with a well-formed, honest shape rather than a raw error/stack.
    console.error('handleOnline: D1 read failed:', error);
